...
Там задаются Wi-Fi, координаты центра радара, единицы измерения, автоматический
или ручной выбор аэропортов и ВПП, режим символов, состав подписей самолётов,
яркость и опциональная подложка. Селектор символов показывает оба варианта прямо
на странице. Выберите `None` для обычного радара без карты либо `Stadia Alidade
Smooth Dark` и укажите Stadia Maps API key. После сохранения плата
перезагрузится.
