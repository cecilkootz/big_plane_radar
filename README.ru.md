...
4. Оставьте ключ под рукой для следующего шага. Не публикуйте его в скриншотах
   или коммитах.
...
При первом запуске с картой дисплей скачивает четыре изображения. Не отключайте
питание и дождитесь, пока `TILE CACHE` завершит тайл `4/4`: после этого радар
откроется сам. Boot log показывает дальность, zoom, размеры, объём PNG, прогресс
скачивания и время декодирования каждого тайла.
...
в setup. Boot log показывает прогресс тайлов `1/4` … `4/4`, `SKIP` при
отключённой карте и `NO KEY`, если Stadia выбрана без ключа.
...
встроенную в полученное изображение. Официальные
[инструкции по API-ключу](https://docs.stadiamaps.com/authentication/) и
[документация Static Maps](https://docs.stadiamaps.com/static-maps/).
