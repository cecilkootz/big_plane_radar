...
- сохранение Wi-Fi, центра радара, единиц измерения, отображения аэропортов,
  состава подписей, яркости карты и дальности в NVS;
...
- символ вертолёта целиком поворачивается по курсу, а хвост продолжает указывать
  в сторону, обратную направлению движения;
...
Там задаются Wi-Fi, координаты центра радара, единицы измерения, отображение
аэропортов и ВПП, состав подписей самолётов, яркость и опциональная подложка.
Выберите `None` для обычного радара без карты либо `Stadia Alidade Smooth Dark`
и укажите Stadia Maps API key. После сохранения плата перезагрузится.
...
репозитория и затем возвращает координаты в локальную страницу. Для этого
сценария нужно включить GitHub Pages из корня репозитория.
