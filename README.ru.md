...
- свайп вверх или вниз прокручивает правый список самолётов;
...
- управление тачем: тап по радару меняет дальность, свайп прокручивает список
  самолётов, тап по строке включает или выключает его трек, длинное нажатие
  открывает setup portal;
...
- опциональная интеграция с Home Assistant через MQTT: сенсоры самолётов через
  MQTT discovery плюс управление дальностью, яркостью карты и питанием экрана;
- консервативные настройки RGB LCD для этой панели: PCLK `13 MHz` и RGB bounce
  buffer `800 * 10`.

## Интеграция с Home Assistant

Прошивка может публиковать данные в MQTT-брокер и появляется в Home Assistant
автоматически через MQTT discovery. Включите её в setup portal в разделе
**Home Assistant**, указав адрес брокера, порт и учётные данные, например
пользователя аддона Mosquitto.

Устройство `Big Plane Radar` предоставляет:

- сенсоры: число самолётов, позывной ближайшего самолёта с атрибутами
  hex, типа, скорости и вертикальной скорости, дистанцию и высоту
  ближайшего самолёта, Wi-Fi RSSI и IP-адрес;
- управление: пресет дальности радара, яркость карты и включение/выключение
  экрана.

Сенсоры обновляются с каждым циклом ADS-B, примерно раз в 5 секунд.
Смена яркости карты заново скачивает четыре кешированных вида карты,
поэтому подложка может на несколько секунд исчезнуть. Касание
выключенного экрана только включает его обратно, не срабатывая как нажатие.

Без MQTT камера `generic`, направленная на
`http://plane-radar.local/screenshot.bmp`, также даёт живое изображение
экрана в дашборде.
...
а также MQTT-подключение к Home Assistant.
